...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (j=1; j<SIZE-1; j+=1) {												//TRY LOOP FLIP 1
		for (i=1; i<SIZE-1; i+=1 ) {
			/* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			p = pow(convolution2D(i, j, input, horiz_operator), 2) + 		//Strength Reduction 2
	...
	for (i=1; i<SIZE-1; i++) {
		for ( j=1; j<SIZE-1; j++ ) {
			t = pow((output[i*SIZE+j] - golden[i*SIZE+j]),2);		//TRY Common Subexpression Elimination 1
			PSNR += t;													//POSSIBLE LOOP UNROLLING 2
		}
	}
