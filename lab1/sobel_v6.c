...
// int convolution2D(const int posy,const int posx, const unsigned char *input,const  char operator[][3]) {
// 	int res;
// 	register int episize2;
// 	episize2=posy*SIZE+posx;
// 	res = 0;
// 	res = input[episize2 - SIZE - 1] * operator[0][0] + input[episize2 - SIZE] * operator[0][1] + input[episize2 - SIZE + 1] * operator[0][2] + input[episize2 - 1] * operator[1][0] + input[episize2] * operator[1][1] + input[episize2 + 1] * operator[1][2] + input[episize2 + SIZE - 1] * operator[2][0] + input[episize2 + SIZE] * operator[2][1] + input[episize2 + SIZE + 1] * operator[2][2];
// 	
// 	return(res);
// }
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	register int episize1,episize2;
	unsigned int p;
	int res;
...
// 			convolution2D(i, j, input, vert_operator)
			p = pow(input[episize2 - SIZE - 1] * horiz_operator[0][0] + input[episize2 - SIZE] * horiz_operator[0][1] + input[episize2 - SIZE + 1] * horiz_operator[0][2] + input[episize2 - 1] * horiz_operator[1][0] + input[episize2] * horiz_operator[1][1] + input[episize2 + 1] * horiz_operator[1][2] + input[episize2 + SIZE - 1] * horiz_operator[2][0] + input[episize2 + SIZE] * horiz_operator[2][1] + input[episize2 + SIZE + 1] * horiz_operator[2][2], 2) +                  pow(  input[episize2 - SIZE - 1] * vert_operator[0][0] + input[episize2 - SIZE] * vert_operator[0][1] + input[episize2 - SIZE + 1] * vert_operator[0][2] + input[episize2 - 1] * vert_operator[1][0] + input[episize2] * vert_operator[1][1] + input[episize2 + 1] * vert_operator[1][2] + input[episize2 + SIZE - 1] * vert_operator[2][0] + input[episize2 + SIZE] * vert_operator[2][1] + input[episize2 + SIZE + 1] * vert_operator[2][2]  , 2);
			...
			episize=i*SIZE;
			if (res > 255)
				output[episize + j] = 255;      
			else
				output[episize + j] = (unsigned char)res;
		}
	}
	...
	for (i=1; i<SIZE-1; i++) {
		...
		for ( j=1; j<SIZE-1; j+=4) {
			PSNR +=pow((output[episize+j] - golden[episize+j]),2) + pow((output[episize+j+1] - golden[episize+j+1]),2) +    pow((output[episize+j+2] - golden[episize+j+2]),2) + pow((output[episize+j+3] - golden[episize+j+3]),2) ;
		}
	}
