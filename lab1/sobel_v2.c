...
int convolution2D(int posy, int posx, const unsigned char *input, char operator[][3]) {
	...
	res = 0;
	res = input[(posy - 1)*SIZE + posx - 1] * operator[0][0] + input[(posy -1)*SIZE + posx] * operator[0][1] + input[(posy -1)*SIZE + posx + 1] * operator[0][2] + input[posy*SIZE + posx - 1] * operator[1][0] + input[posy*SIZE + posx] * operator[1][1] + input[posy*SIZE + posx + 1] * operator[1][2] + input[(posy + 1)*SIZE + posx - 1] * operator[2][0] + input[(posy + 1)*SIZE + posx] * operator[2][1] + input[(posy + 1)*SIZE + posx + 1] * operator[2][2];
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1) {
			...
			if (res > 255)
				output[i*SIZE + j] = 255;      
			else												//TRY Common Subexpression Elimination 4
	...
	for (i=1; i<SIZE-1; i++) {
		for ( j=1; j<SIZE-1; j+=4) {
										    //TRY Common Subexpression Elimination 1
			PSNR +=pow((output[i*SIZE+j] - golden[i*SIZE+j]),2) + pow((output[i*SIZE+j+1] - golden[i*SIZE+j+1]),2) +    pow((output[i*SIZE+j+2] - golden[i*SIZE+j+2]),2) + pow((output[i*SIZE+j+3] - golden[i*SIZE+j+3]),2) ;
		}
	}
